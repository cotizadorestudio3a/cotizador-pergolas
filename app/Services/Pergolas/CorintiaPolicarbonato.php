...
class CorintiaPolicarbonato extends PergolaBase
{
    ...
    public $cantidad_policarbonato;
    public $cantidad_union_policarbonato;
    public $cantidad_cinta_filtro;
    public $cantidad_tornillo_capuchon;
    ...
    public $unidades_union_policarbonato;
    public $unidades_cinta_filtro;
    ...
    public $precio_policarbonato;
    public $precio_union_policarbonato;
    public $precio_cinta_filtro;
    public $precio_tornillo_capuchon;
    ...
    private function calcularCantidades()
    {
        ...
        $this->cantidad_policarbonato = $this->area;
        $this->cantidad_union_policarbonato = ($this->numeroVigas * $this->largoVigas) / 2;
        $this->cantidad_cinta_filtro = $this->largoVigaPrincipal * 2;
        $this->cantidad_tornillo_capuchon = $this->cantidad_cinta_filtro / 1;
    ...
    private function calcularUnidades()
    {
        ...
        $this->unidades_union_policarbonato = $this->cantidad_union_policarbonato / 11.8;
        $this->unidades_cinta_filtro = $this->cantidad_cinta_filtro / 5;
    ...
    private function definirPrecios()
    {
        ...
        $this->precio_policarbonato = $materialPrices['policarbonato'] ?? 15;
        $this->precio_union_policarbonato = $materialPrices['union_policarbonato'] ?? 85;
        $this->precio_cinta_filtro = $materialPrices['cinta_filtro'] ?? 36;
        $this->precio_tornillo_capuchon = $materialPrices['tornillo_capuchon'] ?? 0.9;
    ...
    private function calcularTotales()
    {
        $this->total_pergola = (
            ...
            $this->precio_policarbonato * $this->cantidad_policarbonato +
            $this->precio_union_policarbonato * $this->unidades_union_policarbonato +
            $this->precio_cinta_filtro * $this->unidades_cinta_filtro +
            $this->precio_tornillo_capuchon * $this->cantidad_tornillo_capuchon +
    ...
    public function obtenerDetalleMateriales(): array
    {
        $materiales = [
            ...
            'Unión Policarbonato' => 'union_policarbonato',
            'Cinta Filtro' => 'cinta_filtro',
            'Tornillo Capuchón' => 'tornillo_capuchon',
            'Alumband' => 'alumband',
    ...
    private function processAllServicesForPDF(array $allServices): array
    {
        ...
        foreach ($allServices as $index => $service) {
            ...
            $servicios[] = [
                'tipo' => 'Pergola de corintia Policarbonato',
                'medidas' => [
    ...
    {
        // Obtener nombre de la variante
        $variantName = 'Policarbonato'; // Valor por defecto
        if (isset($this->data['variant_id'])) {
            $variant = \App\Models\ServiceVariants::find($this->data['variant_id']);
            $variantName = $variant ? $variant->name : 'Policarbonato';
        }
    ...
    public function obtenerPDFOrdenProduccion(): string
    {
        ...
        $data = [
            ...
            'titulo' => [
                'titulo_servicio' => 'Pergola con corintia Policarbonato',
            ],
            'cotizacion' => [
                'numero_cotizacion' => $this->data['quotation_id'] ?? 'COT-' . now()->timestamp,
                'fecha_emision' => now()->format('d/m/Y'),
